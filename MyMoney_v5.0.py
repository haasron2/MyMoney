...
#st.write("ron haas")
      ...
      #   this display the main query data
      #########--------------------------------
def show_data_frame(query_name ):

            print(query_name)   
            app_query = ApplQuery.open_appl_query(None,query_name)
            app_query.run_query_data()
            df_query_result = app_query.df_result

            app_query.find_detail_queries()
            df_detail_queries= app_query.detail_queries_df
            df_detail_queries_names = df_detail_queries['query_name']
            color_column_name_list = app_query.get_values_list_for_param('color_column')

            
            if len(color_column_name_list) >0 :
               color_column_name = color_column_name_list[0]
            else :      
               color_column_name = None

            print(color_column_name)
   
      # . Define the styling function to make the green red affect for minus plus debit ammounts 
            def highlight_by_column(column_to_check):
                """
                Returns a styling function that applies color based on the specified column's value.
                """
                def color_rows_by_value(row):
                    """
                    The actual styling function used by Pandas Styler (axis=1).
                    It uses the 'column_to_check' variable captured from the outer function.
                    """
                    value = row[column_to_check]
                    
                    # 1. Define the color based on the value in the specified column
                    if value < 0:
                        color = 'background-color: #ffcccc'  # Light Red (Negative)
                    elif value > 0:
                        color = 'background-color: #ccffcc'  # Light Green (Positive)
                    else:
                        color = '' # Default background
                        
                    # 2. Return a list of the chosen color, one for every column in the row
                    return [color] * len(row)

                return color_rows_by_value
            # --- End of function definition ---

            # Call the wrapper function to get the specialized styling function
            #styler_func = highlight_by_column('debit_amount')

            # Apply the specialized function


            if color_column_name is not None :
              styler_func = highlight_by_column(color_column_name)
              styled_df = df_query_result.style.apply(styler_func, axis=1)
              selection_event = st.dataframe(
                 styled_df,
                 selection_mode="single-row", # or "single-row" for one selection multi-row
                 width='stretch',
                 # Optional: Rerun the app immediately when a selection is made
                 on_select="rerun" 
               )
            else :
              selection_event = st.dataframe(
            ...
            ##edited_df = st.data_editor(df_query_result,
            ##                           num_rows="dynamic",
            ##                           key="task_editor")
            ...
            with col1:
                # 2. Add a standard Streamlit button outside the editor
               #if st.button("💾 Save Changes to Database"):
                if st.button("open detail "): 
                   # 3. Define the action to take when the button is pressed
                   st.session_state.current_view = 'detail'          
                   # You would typically call a function here to update your persistent storage
                   # (e.g., SQL database, API endpoint, or file).
                   st.success("open detail")
                   st.write("open detail:")
                   
            with col2:
                selected_detail_query_name = st.selectbox(
                   'Select a value from Column 1 edited text for   github branch features 2',
                     options=df_detail_queries_names)

                
            with col3:
                if st.button("open plot "):
                  st.success("go to plot")
           #########-----end show dataframe-----------------
...
def show_home_page() :
      ...
      st.write("This is the main screen content.")

      # 1. Define the columns
      # We create two columns of equal width (implicitly, as we don't pass a ratio list)
      col1, col2  = st.columns(2)
      with col1:
         # we select the query categories 
         mq1 = MyQuery ( mydbcon,"select category_name from appl_query_categories",None)
         mq1.execute_pandas_query()
         df_category_name = mq1.query_pandas_result_df
         #print(df_result)

      #   df_options = df_result['category_name']
         selected_category_name = st.selectbox(
             'Select a value from Column 1',
             options=df_category_name
         )
         
         st.write(f"You selected: **{selected_category_name}**")
         args = [selected_category_name]
         mq2 = MyQuery ( mydbcon,"select query_name  from appl_query where query_type ='primary' and query_category = ?",args)
         mq2.execute_pandas_query()
         df_query_name = mq2.query_pandas_result_df
         
      with col2:
         selected_query_name = st.selectbox(
             'Select a value from Column 1',
             options=df_query_name
         )

         st.write(f"You selected: **{selected_query_name}**")

       # start the main function that shows the dataframe 
      show_data_frame(selected_query_name)
...
def show_detail_page() :
     #st.session_state.current_view = 'detail'
     #st.write("This is the detail screen content.")
     if st.button("return home "): 
             # 3. Define the action to take when the button is pressed
          
             # You would typically call a function here to update your persistent storage
             # (e.g., SQL database, API endpoint, or file).
             st.success("return home")
             st.write("return home")
             st.session_state.current_view = 'home'
             

#_____________________________  start main program _________________________
